...
public class AppPreferences { 
    ...
    public AppPreferences(AppLogger _log) {
        ...
        this._log=_log;
        if (null != this._log)
            _log.info("Logger successfully linked to AppPreferences");        
    }     
    ...
    public InfrastructureInfo[] getEnabledInfrastructures() {
        ...
        for(int i=0,h=0; i<getNumInfrastructures(); i++) {
            ...
            if(appInfrastructuresInfo.get(i).getEnableInfrastructure().equalsIgnoreCase("yes")) {
                ...
                // GridEngine supports a list of WMSes/adaptors as an array of strinigs
                ...
                if(getAcronymInfrastructure(i).equalsIgnoreCase("unicore")) {
                    // Multi-infrastructure support (field mapping)
                    // UNICORE infrastructure requires to specify:
                    // * Keystore file -> bdiiHost first  field of the given ';' separated string
                    // * Password      -> bdiiHost second field of the given ';' separated string
                    // * JSAGA adaptor -> wmsHosts
                    String unicoreItems[]  = getBdiiHost(i).split(";");
                    int    numUnicoreItems = unicoreItems.length;
                    String keyStore = "";
                    String password = "";
                    if(numUnicoreItems>0) keyStore=unicoreItems[0].trim();
                    if(numUnicoreItems>1) password=unicoreItems[1].trim();
                    ...
                    infrastructuresInfo[h++] = new InfrastructureInfo( getAcronymInfrastructure(i)
                                                                      ...
                                                                      ,wmsHostList);
                    if(_log !=null)
                        _log.info("Unicore infrastructure: "
                               +LS+" keyStore:      '"+ keyStore    +"'"
                               +LS+" password:      '"+ password    +"'"
                               +LS+" JSAGA adaptor: '"+ wmsHostList +"'"
                                 );
                }
                else if(getAcronymInfrastructure(i).equalsIgnoreCase("garuda")) {
                    // Multi-infrastructure support (field mapping)
                    // GARUDA infrastructure requires to specify:
                    // * VO Group         -> pxRobotVO
                    // * VO Role          -> pxRobotRole
                    // * JSAGA adaptor    -> wmsHosts
                    // * Robot proxy Host -- pxServerHost
                    // * Robot proxy Port -- pxServerPort
                    // * Robot proxy Id   -- pxRobotId
                    ...
                    if(_log !=null)
                        _log.info("GARUDA infrastructure: "
                               +LS+" pxServerHost:  '" + getPxServerHost(i) +"'"
                               +LS+" pxServerPort:  '" + getPxServerPort(i) +"'"
                               +LS+" pxRobotId:     '" + getPxRobotId   (i) +"'"
                               +LS+" VO Group:      '" + getPxRobotVO   (i) +"'"
                               +LS+" VO Role:       '" + getPxRobotRole (i) +"'"
                               +LS+" JSAGA adaptor: '" + wmsHostList        +"'"
                                 ); 
                }
                else if(getAcronymInfrastructure(i).equalsIgnoreCase("ourgrid")) {
                    // Multi-infrastructure support (field mapping)
                    // OURGRID infrastructure requires to specify:
                    // * Username      -> bdiiHost first  field of the given ';' separated string
                    // * Password      -> bdiiHost second field of the given ';' separated string
                    // * JSAGA adaptor -> wmsHosts
                    String ourgridItems[]  = getBdiiHost(i).split(";");
                    int    numOurGridItems = ourgridItems.length;
                    String username = "";
                    String password = "";
                    if(numOurGridItems>0) username=ourgridItems[0].trim();
                    if(numOurGridItems>1) password=ourgridItems[1].trim();
                    ...
                    infrastructuresInfo[h++] = new InfrastructureInfo( getAcronymInfrastructure(i)
                                                                      ...
                                                                      ,wmsHostList);
                    if(_log !=null)
                        _log.info("OURGRID infrastructure: "
